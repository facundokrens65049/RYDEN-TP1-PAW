...
public interface ListingService {
    ...
    /**
     * Bookable wall days for the listing date picker: same as {@link ***REMOVED***getBookableWallAvailabilityPeriods(long)} then
    ...
    /**
     * Publication: each period start must be on or after {@link ***REMOVED***getPublicationMinAvailabilityFirstWallDay}.
     *
    ...
    /**
     * {@code sort} argument for {@link ***REMOVED***buildOwnerListingSearchCriteria} when loading the counterparty profile
    ...
    /**
     * Same as {@link ***REMOVED***buildOwnerListingSearchCriteria(long, List, List, List, BigDecimal, BigDecimal, List, List, String, int, String)}
    ...
    /** {@code app.reservation.pickup-lead-hours} (same source as {@link ReservationService***REMOVED***getConfiguredPickupLeadHours()}). */
    int getConfiguredPickupLeadHours();

    /**
     * Publication rule: availability dates must lie within {@link ***REMOVED***getConfiguredMaxAvailabilityForwardWallDays()}
     * of “today” in {@link AvailabilityPeriod***REMOVED***WALL_ZONE}.
     */
    void validatePublicationAvailabilityAgainstWallCalendar(List<AvailabilityPeriod> periods);

    /**
     * For each active listing of the owner, sets status to {@link Listing.Status***REMOVED***PAUSED_DUE_TO_LACK_OF_CBU} and
