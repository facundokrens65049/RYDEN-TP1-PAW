...
public interface UserService {
    ...
    /** Loads {@link User***REMOVED***getPasswordHash()} for Spring Security; empty if unknown email or no password set. */
    Optional<User> findByEmailForAuthentication(final String email);

    /**
     * Role keys as stored in {@code user_roles.role} (same as {@link ar.edu.itba.paw.models.security.UserRole***REMOVED***name()}).
     */
    ...
    /**
     * {@code birthDate} may be {@code null} to clear. Must not be after today in {@link ar.edu.itba.paw.models.domain.AvailabilityPeriod***REMOVED***WALL_ZONE}.
     */
     ...
     * {@link ***REMOVED***clearProfileDocument(long, UserDocumentType)}.
     */
    ...
    /**
     * Locale for mail when the user has a persisted {@link User***REMOVED***getLatestLocaleTag()}; otherwise {@code fallback}
